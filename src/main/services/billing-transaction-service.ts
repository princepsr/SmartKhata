...
export class BillingTransactionService {
  ...
  async createSale(input: FinalizeBillInput): Promise<{ id: number; billNumber: string }> {
    ...
    return databaseManager.transaction(() => {
      ...
      // 8. Update Customer Ledger (Udhaar)
      if (input.customerId && (input.paymentReceived || 0) < calculation.grandTotal) {
        const balance =
          Math.round((calculation.grandTotal - (input.paymentReceived || 0)) * 100) / 100;
        this.customerRepo.updateBalance(input.customerId, balance);
      }
