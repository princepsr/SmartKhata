...
### Intelligent Tax Splitting (State-Code Detection)

SmartKhata automatically determines the tax type by comparing the **State Code** (first 2 digits) of the Shop's GSTIN and the Customer/Supplier's GSTIN.

1.  **Intrastate (Within State)**:
    - **Trigger**: State codes match (e.g., both start with `29`).
    - **Split**: `CGST = GST / 2` and `SGST = GST / 2`.
2.  **Interstate (Outside State)**:
    - **Trigger**: State codes differ (e.g., `29` vs `07`).
    - **Split**: `IGST = GST (100%)`.
3.  **Fallback**: If GSTINs are missing, the system defaults to the `supply_type` configured in settings (defaulting to `intrastate`).

### GST Compliance (Mandatory Fields)

To ensure legally valid invoices, the following fields are **strictly mandatory** in Shop Settings whenever GST is enabled:
