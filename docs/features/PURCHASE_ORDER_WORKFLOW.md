...
## 2. The Integrated Procurement Lifecycle

SmartKhata connects suppliers, orders, and inventory into a single, automated chain of custody.

```mermaid
graph TD
    S[Supplier Management] -->|Plan Order| PO[Purchase Order]
    PO -->|Awaiting Delivery| PO_PENDING{Status: PENDING}
    PO_PENDING -->|Physical Receiving| P[Purchase Invoice]
    P -->|Update Status| PO_RECEIVED{Status: RECEIVED}
    P -->|Financial Link| SL[Supplier Ledger]
    P -->|Inventory Link| I[Product Stock]
    I -->|Audit Trail| IL[Inventory Logs]
```

### Step 1: Planning (Purchase Order)

- **Entity**: `Purchase Order`
- **Impact**: Zero financial or inventory impact. Used primarily for tracking vendor commitments and price verification.

### Step 2: Realization (PO to Purchase)

- **Action**: User converts a PO to a Purchase via the "Inward" flow.
- **Auto-Fill**: The system maps PO items, quantities, and planned rates to the Purchase form, reducing manual entry errors.

### Step 3: Persistence (The Purchase Handshake)

When a Purchase is recorded:

1.  **Inventory**: `product.stock_qty` is incremented.
2.  **Audit**: An `inventory_logs` entry is created with reason `PURCHASE`.
3.  **Accounts Payable**: The Supplier's `balance_due` increases.
4.  **Ledger**: A row is added to `supplier_ledger` for financial tracking.
5.  **PO Closure**: The originating PO status is flipped to `RECEIVED`.
