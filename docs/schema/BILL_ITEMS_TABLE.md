...
Day 1: Sell "Coca Cola 500ml" for ₹40.00
       → Stored in bill_items: product_name_snapshot = "Coca Cola 500ml", unit_price = 40.00

Day 2: Product renamed to "Coke 500ml" and price increased to ₹45.00
       → products table updated: name = "Coke 500ml", sale_price = 45.00
...
| Column        | Type | Constraints            | Description                             |
| ------------- | ---- | ---------------------- | --------------------------------------- |
| `unit_price`  | REAL | NOT NULL, >= 0         | Price per unit at time of sale (Rupees) |
| `gst_percent` | REAL | NOT NULL, >= 0, <= 100 | GST rate at time of sale (Percentage)   |
| `line_total`  | REAL | NOT NULL, >= 0         | Total for this line item (Rupees)       |
...
// Subtotal (before GST)
const subtotal = unitPrice * quantity; // 80.00

// GST amount
const gstAmount = (subtotal * gstPercent) / 100; // 14.40

// Line total
const lineTotal = subtotal + gstAmount; // 94.40
```
...
```
Day 1: Sell "Coca Cola 500ml" for ₹40.00
       → bill_items stores: product_id = 101, product_name_snapshot = "Coca Cola 500ml", unit_price = 40.00
...
const items = [
  {
    ...
    quantity: 2,
    unit_price: 40.0, // ₹40.00
    gst_percent: 18.0, // 18%
    line_total: 94.4, // ₹94.40
  },
  {
    bill_id: 1,
    product_id: 102,
    ...
    quantity: 1,
    unit_price: 60.0, // ₹60.00
    gst_percent: 0, // No GST
    line_total: 60.0, // ₹60.00
  },
];
...
billItems.forEach((item) => {
  console.log(
    `${item.product_name_snapshot} x ${item.quantity} @ ₹${item.unit_price / 100} = ₹${item.line_total / 100}`
  );
});
```
...
const productSales = db.queryAll(
  ...
  SELECT 
    ...
    SUM(bi.line_total) as total_sales_rupees
  FROM bill_items bi
  JOIN products p ON bi.product_id = p.id
  WHERE DATE(bi.created_at) BETWEEN ? AND ?
  GROUP BY bi.product_id
  ORDER BY total_sales_rupees DESC
  LIMIT 10
`,
  [startDate, endDate]
);
