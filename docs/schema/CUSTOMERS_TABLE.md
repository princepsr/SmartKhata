...
| Column        | Type | Constraints         | Description                   |
| ------------- | ---- | ------------------- | ----------------------------- |
| `balance_due` | REAL | NOT NULL, DEFAULT 0 | Outstanding balance in Rupees |

**Design Choices:**

**REAL Storage (Rupees):**

- Stored as Decimal in Rupees for precision and simplicity
...
| Value    | Rupees   | Meaning                    |
| -------- | -------- | -------------------------- |
| `500.0`  | ₹500.00  | Customer owes ₹500         |
| `0.0`    | ₹0.00    | No outstanding balance     |
| `-200.0` | -₹200.00 | Customer paid ₹200 advance |
...
| Column       | Type    | Constraints      | Description                      |
| ------------ | ------- | ---------------- | -------------------------------- |
| `is_active`  | INTEGER | NOT NULL, 0 or 1 | Soft delete flag (0=Inactive)    |
...
// Result: balance_due increases by 500.00
```
...
// Result: balance_due increases by 200.00
```
...
// Result: balance_due decreases by 500.00
```
...
// Result: balance_due becomes negative (-1000.00)
```
...
SELECT
  ...
  SUM(balance_due) as total_due_rupees
FROM customers
WHERE is_active = 1 AND balance_due > 0;
```
...
SELECT
  ...
  name,
  phone,
  ABS(balance_due) as advance_rupees
FROM customers
WHERE is_active = 1 AND balance_due < 0
...
| -------------------- | ---------------------------------------- | ----------------------------------------- |
| **Balance tracking** | Single `balance_due` field (REAL Rupees) | Simple, precise, handles advance payments |
