...
**See:** [`SERVICE_LAYER_RULES.md`](../development/SERVICE_LAYER_RULES.md), [`IPC_SERVICE_MAPPING.md`](../security/IPC_SERVICE_MAPPING.md)
...
### 8. Domain Separation (Medical vs Kirana) ✅

**Decision:** Dynamic UI/Service pivoting based on `business_type` and feature toggles.

**Rationale:**

- **UX Specialization**: A pharmacist needs salt-search; a grocer needs a weighing scale. Providing both to everyone clutters the interface.
- **Code Reuse**: Both domains share 80% of the core POS engine (Billing, Tax, Reports).
- **Maintainability**: Feature-specific logic is encapsulated in `MedicalService` and `KiranaService` rather than a single monolithic "POS Service".

**Implementation:**

- **State Layer**: `useAppSettingsStore` holds the current `setupMode` and `activeFeatures`.
- **UI Layer**: High-order components and conditional rendering (e.g., `{mode === 'MEDICAL' && <SaltSearch />}`) control visibility.
- **Service Layer**: Specialized services are injected/initialized based on mode, preventing unnecessary resource usage (like SerialPort listeners for non-Kirana stores).
